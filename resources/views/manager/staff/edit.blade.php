...
@section('page-subtitle', $staff->user->name)
...
@section('content')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('manager.staff.update', $staff) }}" class="space-y-6">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-border p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-secondary mb-1.5">Phone</label>
            ...
            <div>
                ...
                <input type="text" name="id_number" value="{{ old('id_number', $staff->id_number) }}"
                    class="w-full text-sm border border-border rounded-xl px-3 py-2 bg-surface focus:outline-none focus:ring-2 focus:ring-brand-400
        ...
        <div class="bg-white rounded-2xl border border-border p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-semibold text-secondary mb-1.5">Status <span class="text-danger">*</span></label>
                    <select name="status" required
                        class="w-full text-sm border border-border rounded-xl px-3 py-2 bg-surface focus:outline-none focus:ring-2 focus:ring-brand-400
                               @error('status') border-danger @enderror">
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                    class="w-full text-sm border border-border rounded-xl px-3 py-2 bg-surface focus:outline-none focus:ring-2 focus:ring-brand-400
                           @error('notes') border-danger @enderror">{{ old('notes', $staff->notes) }}</textarea>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('manager.staff.show', $staff) }}"
