...
@section('title', 'Import Tenants')
@section('page-title', 'Import Tenants')
@section('page-subtitle', 'Upload a CSV or Excel file to add multiple tenants at once')
...
<div class="max-w-2xl space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-border p-6 space-y-6">
        ...
        <div class="flex items-center justify-between">
            <h2 class="text-base font-semibold text-primary">Upload Tenant File</h2>
        ...
        <form action="{{ route('manager.bulk-upload.tenants.import') }}" method="POST" enctype="multipart/form-data" id="tenantUploadForm">
            @csrf
            ...
            <div id="dropZone"
                ...
                <svg id="dropIcon" class="w-10 h-10 text-muted mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button type="submit"
                    class="mt-5 w-full inline-flex items-center justify-center gap-2 bg-brand-600 hover:bg-brand-500 text-white text-sm font-semibold px-4 py-3 rounded-xl transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-border">
                    @php
                    $columns = [
                        ['name',                   true,  'Full name of the tenant'],
                        ['email',                  true,  'Must be unique — used as login'],
                        ['phone',                  true,  'e.g. 0712345678'],
                        ['id_number',              true,  'National ID or Passport — must be unique'],
                        ['emergency_contact_name', false, 'Next of kin name'],
                        ['emergency_contact_phone',false, 'Next of kin phone'],
                        ['occupation',             false, 'Tenant\'s job title'],
                        ['employer',               false, 'Employer name'],
                        ['notes',                  false, 'Any internal notes'],
                    ];
                    @endphp
